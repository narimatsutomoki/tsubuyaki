...
public class TsubuyakiController {
    ...
    // 検索結果画面を表示
    @GetMapping("/find")
    String findTsubuyakiListByWord(@RequestParam("word") String word,Model model) {
        List<Tsubuyaki> listByWord = ts.getTsubuyakiByWord(word); // 該当のつぶやきを取得
        model.addAttribute("tsubuyakiListByWord", listByWord); // モデル属性にリストをセット
        return "found"; // 検索結果画面を返す
    }
